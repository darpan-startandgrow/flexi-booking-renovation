...
class BM_ResourcePool {
	...
	public function create_pool( string $name, int $total_capacity, string $allocation_rule = 'shared', string $description = '' ) {
		global $wpdb;
		$table = $this->activator->get_db_table_name( 'RESOURCE_POOL' );
		$result = $wpdb->insert(
			$table,
			[
	...
	public function update_pool( int $pool_id, array $data ): bool {
		...
		$formats[]         = '%s';
		return (bool) $wpdb->update( $table, $set, [ 'id' => $pool_id ], $formats, [ '%d' ] );
	}
	...
	public function delete_pool( int $pool_id ): bool {
		...
		return (bool) $wpdb->delete(
			$this->activator->get_db_table_name( 'RESOURCE_POOL' ),
			[ 'id' => $pool_id ],
			[ '%d' ]
		);
	}
	...
	public function get_all_pools( $status = 1 ): array {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $table WHERE status = %d ORDER BY id ASC", $status ) ) ?: [];
	}
	...
	public function unlink_service_from_pool( int $service_id, int $pool_id ): bool {
		global $wpdb;
		return (bool) $wpdb->delete(
			$this->activator->get_db_table_name( 'SERVICE_RESOURCE_POOL' ),
	...
	public function get_pools_for_service( int $service_id ): array {
		global $wpdb;
		$link_table = $this->activator->get_db_table_name( 'SERVICE_RESOURCE_POOL' );
		$pool_table = $this->activator->get_db_table_name( 'RESOURCE_POOL' );
	...
	public function get_pool_availability( int $pool_id, string $date ): int {
		...
		global $wpdb;
		$link_table  = $this->activator->get_db_table_name( 'SERVICE_RESOURCE_POOL' );
		$slot_table  = $this->activator->get_db_table_name( 'SLOTCOUNT' );

		// Total consumed seats across all linked services on this date
		$consumed = (int) $wpdb->get_var( $wpdb->prepare(
