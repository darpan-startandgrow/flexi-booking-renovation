...
class Booking_Management_Activator {
	...
	public function create_table() {
		...
		$sql        = "CREATE TABLE IF NOT EXISTS $table_name (
			...
			`is_visible_frontend` int(11) DEFAULT 1,
		...
		$sql        = "CREATE TABLE IF NOT EXISTS $table_name (
			...
			`created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
			PRIMARY KEY (`id`),
			UNIQUE KEY `unique_svc_global_extra` (`service_id`, `global_extra_id`)
		)$charset_collate;";
		dbDelta( $sql );

		$this->add_error_column_to_emails();
		$this->add_error_column_to_failed_transactions();
		$this->add_extra_type_to_extraslotcount();
	...
	private function add_extra_type_to_extraslotcount() {
		global $wpdb;
		...
		if ( empty( $row ) ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.SchemaChange -- Required for plugin activation migration.
			$wpdb->query( "ALTER TABLE `{$table_name}` ADD `extra_type` varchar(10) DEFAULT 'local' AFTER `is_active`" );
		}
	}
